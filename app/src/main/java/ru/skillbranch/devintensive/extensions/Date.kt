...
fun getPluralForm(value: Int, timeUnits: TimeUnits): String {
    val posValue = abs(value) % 100

    return when(posValue) {
        1 -> Plurals.ONE.get(timeUnits)
        in 2..4 -> Plurals.FEW.get(timeUnits)
        0, in 5..19 -> Plurals.MANY.get(timeUnits)
        else -> getPluralForm(posValue % 10, timeUnits)
    }
}
